...
class MessageEnum(Enum):
    ...
    set_project_not_exist=100054,'设置的项目不存在或者已经删除'
    ...
    model_edit_fail=100124,'添加失败'
    model_not_exict=100125,'模块不存在'
    edit_exception=100126,'编辑出现问题'
    ...
    # === 向后兼容别名（拼写修正后保留旧名）===
    login_user_sucess_message = 0, '登录成功！'  # alias of login_user_success_message
    login_user_not_exict_message = 100009, '用户不存在'  # alias of login_user_not_exist_message
    login_user_free_message = 100004, '用户冻结！'  # alias of login_user_freeze_message
    testeveirment_not_exict = 100015, '没有找到测试环境'  # alias of test_environment_not_exist
    project_not_exict = 100016, '无法找不到项目'  # alias of project_not_exist
    email_geshi_error = 100019, '邮箱格式错误'  # alias of email_format_error
    email_exict = 100022, '邮箱已经注册'  # alias of email_exist
    common_is_not_exict = 100025, '编辑的通用配置不存在，请确定'  # alias of common_is_not_exist
    case_not_exict = 100028, '测试用例不存在'  # alias of case_not_exist
    re_is_not_exitc = 100029, '操作的类型不支持'  # alias of operation_type_not_supported
    re_editisnot = 100030, '编辑操作不存在'  # alias of operation_not_exist
    ower_not_free_me = 100041, '自己不能解冻自己'  # alias of owner_not_free_me
    ower_cannot_free_me = 100045, '自己不能冻结自己'  # alias of owner_cannot_free_me
    Interface_edit = 100097, '编辑成功'  # alias of interface_edit
    Interface_edit_fail = 100098, '编辑失败'  # alias of interface_edit_fail