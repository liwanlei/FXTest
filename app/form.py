...
class RegFrom(Form):
    username = StringField('注册用户名', [validators.Length(min=4, max=16, message=u'用户名长度在4-16位'), validators.DataRequired()],render_kw={'placeholder': u'请输入用户名'})
    password = PasswordField('注册密码', [validators.length(min=8, max=16, message=u'密码长度8-16位'), validators.DataRequired()],render_kw={'placeholder': u'请输入密码'})
    se_password = PasswordField('再次输入密码', [validators.length(min=8, max=16, message=u'密码长度8-16位'), validators.DataRequired()],
                             render_kw={'placeholder': u'请输入密码'})
    email=StringField('输入注册邮箱', [ validators.DataRequired()],
...
class Interface_yong_Form(Form):
    yongli_name=StringField('项目', [validators.DataRequired()],render_kw={'placeholder': u'请输入接口项目名称'})
    model_name=StringField('模块', [validators.DataRequired()],render_kw={'placeholder': u'请输入接口模块名称'})
    interface_name = StringField('接口名字', [validators.DataRequired()], render_kw={'placeholder': '请输入接口名称'})
    interface_url = StringField('接口url', [validators.DataRequired()], render_kw={'placeholder': '请输入接口url'})
    interface_meth = StringField('请求方式', [validators.DataRequired()], render_kw={'placeholder': '请输入接口请求方式'})
