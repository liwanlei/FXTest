...
class Interface_yong_Form(Form):#测试用例的表单
    ...
    test_name=StringField(u'用例编号', [validators.DataRequired()],render_kw={'placeholder': u'请输入接口用例编号'})
...
class Set_email_Form(Form):#设置发送邮箱的
    send_email=StringField(u'请输入邮箱', [ validators.DataRequired(message=u'请输入邮箱'),Email(message=u'邮箱格式不对')],
                           render_kw={'placeholder': u'请输入邮箱'})
    password=StringField(u'请输入安全密码：', [validators.DataRequired(message=u'请输入安全密码')],render_kw={'placeholder': u'请输入邮箱安全密码'})
    stmp_email=StringField(u'请输入stmp地址：', [validators.DataRequired(message=u'请输入stmp地址：')],render_kw={'placeholder': u'请输入stmp地址：'})
    port=StringField(u'请输入stmp的端口号：', [validators.DataRequired(message=u'请输入stmp的端口号：')],render_kw={'placeholder': u'请输入stmp的端口号：'})
class Interface_Env(Form):#测试环境
    ...
    envtion=StringField(u'测试环境url', [validators.DataRequired(message=u'请输入测试环境url')], render_kw={'placeholder': u'测试环境url'})
