...
----------------
## 本地调试 安装依赖包后， python manage.py run即可  成功可以访问http://127.0.0.1:5000/home/index项目使用的数据库是sqlite
...
## 待开发功能
### 1.集成到jenkins
### 2.测试用例代码抽离(参考开源的接口测试框架),形成测试平台，用例执行的分离。
### 3.一键将接口测试用例转化为压测脚本，进行压测。转换为Jmeter脚本
### 4.测试用例可以依赖通用参数配置。
### 5.数据库进行调整优化，适配新功能落地。
### 6.增加黑名单功能
