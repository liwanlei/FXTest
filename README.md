...
###  3.增加用例前后置操作的入口(设计中)
###  4.增加用例校验点(设计中)
###  5.用例可以选择是否CI集成(CI环境待开发)
###  6.接口测试覆盖率统计(基于jacoco实现，设计中)
###  7.开发注册入口(后台可配置检验邮箱的格式后缀，对工号进行校验，一个工号只能注册一次,旧用户管理员管理用户处激活)
###  8.注册后邮件通知注册成功，将密码等附上(可不选择，将代码注释掉)
###  9.记录用户登录信息（ip+登录时间）,密码输入5次锁定10分钟 (redis存储)
###  10.考虑增加版本区分，对覆盖率统计增量代码(设计中)。

## [其他版本更新日志](https://github.com/liwanlei/FXTest/blob/master/versions.md)
# 有问题可以联系我，可增加定制开发：QQ或者微信:952943386 email:leileili126@163.com  qq群：194704520  python测试开发群：683894834
