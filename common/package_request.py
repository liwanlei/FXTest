# -*- coding: utf-8 -*-
# @Date    : 2017-07-20 15:13:25
# @Author  : lileilei
'''
requets模块的简单的封装
'''
import requests, json
from config import Interface_Time_Out
from requests import exceptions
from common.system_log import logger

class Request:
    def get(self, url, headers, parms):
        try:
            self.resp = requests.get(url, headers=headers,
                                  params=parms, timeout=Interface_Time_Out)
            self.resp.encoding = 'UTF-8'
            spend = self.resp.elapsed.total_seconds()
            json_response = json.loads(self.resp.text)
            return json_response, spend
        except exceptions.Timeout:
            logger.exception(exceptions.Timeout)
            return {'get请求出错': "请求超时"}, 0
        except exceptions.InvalidURL:
            logger.exception(exceptions.InvalidURL)
            return {'get请求出错': "非法url"}, 0
        except exceptions.HTTPError:
            logger.exception(exceptions.HTTPError)
            return {'get请求出错': "http请求错误"}, 0
        except Exception as e:
            logger.exception(e)
            return {'get请求出错': "错误原因:%s" % e}, 0

    def post(self, url, params, headers):
        data = json.dumps(params)
        try:
            resp = requests.post(url,
                                    data=data,
                                    timeout=Interface_Time_Out,
                                    headers=headers)

            if not 200 <= resp.status_code < 300:
                return {'post请求出错': "状态码返回不是2xx: %s" % resp.status_code}, 0
            json_response = json.loads(resp.text)
            spend = resp.elapsed.total_seconds()
            return json_response, spend
        except exceptions.Timeout:
            logger.info(exceptions.Timeout)
            return {'post请求出错': "请求超时"}, 0
        except exceptions.InvalidURL:
            logger.info(exceptions.InvalidURL)
            return {'post请求出错': "非法url"}, 0
        except exceptions.HTTPError:
            logger.info(exceptions.HTTPError)
            return {'post请求出错': "http请求错误"}, 0
        except Exception as e:
            logger.info(e)
            return {'post请求出错': "错误原因:%s" % e}, 0
    def put(self, url, params, headers):
        data = json.dumps(params)
        try:
            resp = requests.put(url,
                                    data=data,
                                    timeout=Interface_Time_Out,
                                    headers=headers)

            if not 200 <= resp.status_code < 300:
                logger.info(resp.status_code)
                return {'put请求出错': "状态码返回不是2xx: %s" % resp.status_code}, 0
            json_response = json.loads(resp.text)
            spend = resp.elapsed.total_seconds()
            return json_response, spend
        except exceptions.Timeout:
            logger.info(exceptions.Timeout)
            return {'put请求出错': "请求超时"}, 0
        except exceptions.InvalidURL:
            logger.info(exceptions.InvalidURL)
            return {'put请求出错': "非法url"}, 0
        except exceptions.HTTPError:
            logger.info(exceptions.HTTPError)
            return {'put请求出错': "http请求错误"}, 0
        except Exception as e:
            logger.info(e)
            return {'put请求出错': "错误原因:%s" % e}, 0


    def delete(self, url, params, headers):
        try:
            self.rdel_word = requests.delete(url, data=params,
                                             headers=headers,
                                             timeout=Interface_Time_Out)
            json_response = json.loads(self.rdel_word.text)
            spend = self.rdel_word.elapsed.total_seconds()
            return json_response, spend
        except exceptions.Timeout:
            logger.info(exceptions.Timeout)
            return {'delete请求出错': "请求超时"}, 0
        except exceptions.InvalidURL:
            logger.info(exceptions.InvalidURL)
            return {'delete请求出错': "非法url"}, 0
        except exceptions.HTTPError:
            logger.info(exceptions.HTTPError)
            return {'delete请求出错': "http请求错误"}, 0
        except Exception as e:
            logger.info(e)
            return {'delete请求出错': "错误原因:%s" % e}, 0

    def putfile(self, url, params, headers):
        try:
            self.rdata = json.dumps(params)
            me = requests.put(url, self.rdata, headers=headers,
                              timeout=Interface_Time_Out)
            json_response = json.loads(me.text)
            spend = me.elapsed.total_seconds()
            return json_response, spend
        except exceptions.Timeout:
            logger.exception(exceptions.Timeout)
            return {'put请求出错': "请求超时"}, 0
        except exceptions.InvalidURL:
            logger.exception(exceptions.InvalidURL)
            return {'put请求出错': "非法url"}, 0
        except exceptions.HTTPError:
            logger.exception(exceptions.HTTPError)
            return {'put请求出错': "http请求错误"}, 0
        except Exception as e:
            logger.exception(e)
            return {'put请求出错': "错误原因:%s" % e}, 0
